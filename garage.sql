...
CREATE TABLE AspNetUsers (
    Id NVARCHAR(450) NOT NULL PRIMARY KEY,   -- Identyfikator użytkownika
    UserName NVARCHAR(256) NULL,            -- Nazwa użytkownika
    NormalizedUserName NVARCHAR(256) NULL,  -- Znormalizowana nazwa użytkownika
    Email NVARCHAR(256) NULL,               -- Adres e-mail użytkownika
    NormalizedEmail NVARCHAR(256) NULL,     -- Znormalizowany adres e-mail
    EmailConfirmed BIT NOT NULL DEFAULT 0,  -- Flaga potwierdzenia e-mail
    PasswordHash NVARCHAR(MAX) NULL,        -- Hasło zaszyfrowane (hash)
    SecurityStamp NVARCHAR(MAX) NULL,       -- Znacznik zabezpieczeń
    ConcurrencyStamp NVARCHAR(MAX) NULL,    -- Znacznik współbieżności
    PhoneNumber NVARCHAR(MAX) NULL,         -- Numer telefonu
    PhoneNumberConfirmed BIT NOT NULL DEFAULT 0, -- Flaga potwierdzenia numeru telefonu
    TwoFactorEnabled BIT NOT NULL DEFAULT 0,     -- Flaga uwierzytelnienia dwuskładnikowego
    LockoutEnd DATETIMEOFFSET NULL,         -- Data zakończenia blokady użytkownika
    LockoutEnabled BIT NOT NULL DEFAULT 0,  -- Flaga blokady konta
    AccessFailedCount INT NOT NULL DEFAULT 0, -- Liczba nieudanych prób logowania
	FirstName NVARCHAR(MAX) NULL,
    LastName NVARCHAR(MAX) NULL
);

-- Dodanie indeksów dla zoptymalizowanego wyszukiwania
CREATE UNIQUE INDEX IX_AspNetUsers_NormalizedUserName ON AspNetUsers (NormalizedUserName)
WHERE NormalizedUserName IS NOT NULL;
